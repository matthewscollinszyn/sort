        ...
        <motion.div variants={stagger} initial="hidden" animate="visible"
          ...
          {campusPartners.map((p, i) => (
            <motion.div key={p.name} variants={fadeUp} custom={i}
              ...
              <div className={`mb-3 flex h-11 w-11 items-center justify-center rounded-xl transition-colors ${theme === 'dark' ? 'bg-eco-green/10 group-hover:bg-eco-green/15' : 'bg-eco-green/10 group-hover:bg-eco-green/15'
                }`}>
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              {timelineMilestones.map((m, i) => (
                <motion.div key={m.title} variants={fadeUp} custom={i}
...
function ReportTab({ binLocations = [], roomLocations = [], wasteTypes = [], urgencyLevels = [] }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // For now, we'll just send the text data (photo upload would need a separate endpoint)
      const reportData = {
        location,
        notes,
        urgency,
        wasteType,
        photoUrl: photo ? 'photo-captured' : null // Placeholder - file upload needs separate implementation
      };
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
      ...
      <Card theme={theme}>
        <h4 className={`mb-3 flex items-center gap-2 text-sm font-semibold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>
          <MapPin className="h-4 w-4 text-slate-500" /> Bin Location
          <span className="text-red-400">*</span>
        </h4>
            ...
            <div className={`mt-2 max-h-48 overflow-y-auto space-y-0.5 rounded-xl border ${theme === 'dark' ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-white'}`}>
              {filteredLocations.map((b) => (
                <button key={b.id} type="button"
                  onClick={() => { setLocation(b.name); setLocationSearch(''); }}
...
function MapTab() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
      ...
      <Card theme={theme}>
        <div className="flex items-center justify-between mb-4">
          <h3 className={`text-lg font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-900'} flex items-center gap-2`}>
            <MapPinned className="h-5 w-5 text-eco-green" /> Live Bin Map
          </h3>
        ...
        <div className={`mt-4 flex flex-wrap items-center gap-4 text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`}>
          <span className={`font-semibold ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Legend:</span>
            ...
            <Card className="border-eco-green/20" glow theme={theme}>
              ...
              {selectedBin.status === 'full' && (
                <div className="mt-3 flex items-center gap-2 rounded-xl bg-red-400/10 border border-red-400/15 px-4 py-3">
                  ...
                  <p className="text-xs text-red-300">This bin is full and needs immediate attention. Report it to earn eco-points!</p>
                </div>
              )}
            </Card>
          </motion.div>
      ...
      {/* Bin List */}
      <Card theme={theme}>
        <h3 className={`mb-3 text-sm font-semibold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'} flex items-center gap-2`}>
          <Filter className="h-4 w-4 text-slate-500" /> All Campus Bins ({filtered.length})
        </h3>
        <div className="space-y-1">
          {filtered.map((bin) => (
            <button key={bin.id} type="button" onClick={() => setSelectedBin(bin)}
              className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left transition-colors ${selectedBin?.id === bin.id ? 'bg-eco-green/10 border border-eco-green/20' : `${theme === 'dark' ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-100'}`
                }`}>
              <span className={`h-3 w-3 shrink-0 rounded-full ${bin.status === 'full' ? 'bg-red-500' : bin.status === 'half' ? 'bg-amber-400' : 'bg-eco-green'}`} />
              <div className="flex-1 min-w-0">
...
/* ═══════════════════════════════════════════════════════════
   ACTIVITY TAB – All student report history
   ═══════════════════════════════════════════════════════════ */
function ActivityTab({ me, myReports, onRefresh, loading }) {
  const { theme } = useTheme();
