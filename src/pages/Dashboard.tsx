...
import {
    ...
    type LeaderboardEntry, type BinReport, type AssetReport,
} from '../data/reportState';

const fadeUp = {
    hidden: { opacity: 0, y: 20 },
    visible: (i: number = 0) => ({ opacity: 1, y: 0, transition: { delay: i * 0.1, duration: 0.4 } }),
};
...
export default function Dashboard(): JSX.Element {
    ...
    const me: LeaderboardEntry = leaderboard[0] || { rank: 0, name: 'Guest', points: 0, reports: 0 };
            ...
            <div className="flex gap-1 rounded-2xl bg-white p-1 shadow-card overflow-x-auto">
                {TABS.map((tab) => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        className={`relative flex-1 min-w-[5rem] rounded-xl px-4 py-2.5 text-sm font-medium transition-colors ${activeTab === tab.id ? 'text-white' : 'text-eco-muted hover:text-eco-text'
                            }`}>
                        {activeTab === tab.id && (
...
function OverviewTab({ me }: OverviewTabProps): JSX.Element {
    return (
            ...
            <div className="eco-card">
                ...
                <div className="space-y-1.5">
                    {leaderboard.slice(0, 3).map((u) => (
                        <div key={u.rank}
                            className={`flex items-center gap-4 rounded-xl px-4 py-2.5 text-sm ${u.rank === me.rank ? 'bg-eco-green-light font-semibold text-eco-green' : 'hover:bg-slate-50'
                                }`}>
...
function LeaderboardTab({ myRank }: LeaderboardTabProps): JSX.Element {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
            ...
            <div className="space-y-1.5">
                {leaderboard.map((u) => (
                    <div key={u.rank}
                        className={`flex items-center gap-4 rounded-xl px-4 py-2.5 text-sm transition-colors ${u.rank === myRank ? 'bg-eco-green-light font-semibold text-eco-green' : 'hover:bg-slate-50'
                            }`}>
                        <span className="w-6 text-center font-bold">{u.rank}</span>
                        <span className="flex-1">{u.name}</span>
