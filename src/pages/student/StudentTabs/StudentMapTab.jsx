...
import { MapPinned, Trash2, AlertTriangle, X, Filter } from 'lucide-react';
...
export default function StudentMapTab() {
    ...
    useEffect(() => {
        ...
        const unsubStatus = realtimeEvents.subscribe('binStatus.updated', (data) => {
            console.log('📍 [StudentMap] Real-time bin status update received:', data);
        ...
        const unsubCreated = realtimeEvents.subscribe('location.created', () => {
            console.log('📍 [StudentMap] New location created, refreshing...');
            loadMapData();
        });

        const unsubUpdated = realtimeEvents.subscribe('location.updated', () => {
            console.log('📍 [StudentMap] Location updated, refreshing...');
            loadMapData();
        });

        const unsubDeleted = realtimeEvents.subscribe('location.deleted', () => {
            console.log('📍 [StudentMap] Location deleted, refreshing...');
            loadMapData();
        });
                ...
                <div className="space-y-1">
                    {filtered.map((bin) => (
                        <button key={bin.id} type="button" onClick={() => setSelectedBin(bin)}
                            ...
                            <span className={`h-3 w-3 shrink-0 rounded-full ${bin.status === 'full' ? 'bg-red-500' : 'bg-eco-green'}`} />
